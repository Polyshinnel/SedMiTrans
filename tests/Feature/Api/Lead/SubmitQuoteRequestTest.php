...
final class SubmitQuoteRequestTest extends TestCase
{
    ...
    public function test_it_submits_and_repeats_an_identical_request(): void
    {
        ...
        $payload = ['name' => 'Alice', 'phone' => '+7 (999) 123-45-67', 'email' => 'ALICE@example.test', 'cargo' => 'Оборудование', 'route' => 'Москва — Казань'];
    ...
    public function test_it_rejects_invalid_requests_and_conflicting_retries(): void
    {
        ...
        $this->postJson('/api/v1/leads/quote-requests', ['name' => '', 'phone' => 'bad', 'cargo' => '', 'route' => ''], $headers)->assertUnprocessable()->assertJsonValidationErrors(['name', 'phone', 'cargo', 'route']);

        $this->postJson('/api/v1/leads/quote-requests', ['name' => 'Alice', 'phone' => '+79991234567', 'cargo' => 'Груз', 'route' => 'Москва'], $headers)->assertCreated();
        $this->postJson('/api/v1/leads/quote-requests', ['name' => 'Bob', 'phone' => '+79991234567', 'cargo' => 'Груз', 'route' => 'Москва'], $headers)->assertConflict();
    }
}
