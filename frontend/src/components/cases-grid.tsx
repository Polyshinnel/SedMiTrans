...
export function CasesGrid() {
  return (
    <section className={styles.section} aria-label="Список кейсов">
        ...
        {cases.map((item) => (
          <article key={item.slug} className={styles.card}>
            <Anchor href={`/kejsy/${item.slug}`} className={styles.imageLink} aria-label={`Открыть кейс «${item.title}»`}>
              <Image src={item.image} alt={item.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 25vw" className={styles.image} />
            </Anchor>
      ...
      <Pagination total={3} siblings={1} boundaries={1} className={styles.pagination} color="brandOrange" aria-label="Пагинация кейсов" />
    </section>
  );
}
