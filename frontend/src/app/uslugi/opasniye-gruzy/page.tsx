...
export async function generateMetadata() { return buildSeoMetadata(await getSeoPage('dangerous-cargo')); }
...
export default function DangerousCargoPage() {
  return <>
    <section className={styles.hero} style={{ backgroundImage: "url('/images/dangerous-cargo-hero-v2.webp')" }} aria-labelledby="dangerous-hero-title"><div className={styles.heroOverlay} /><div className={styles.heroContent}><Box className={styles.heroBreadcrumbs}><Anchor href="/" underline="never">Главная</Anchor><span aria-hidden="true">/</span><Anchor href="/uslugi" underline="never">Услуги</Anchor><span aria-hidden="true">/</span><Text component="span">Опасные грузы</Text></Box><Title id="dangerous-hero-title" order={1} className={styles.heroTitle}>Опасные грузы <span>по правилам безопасности</span></Title><Text className={styles.heroDescription}>Организуем перевозку опасных грузов с соблюдением международных требований, подбором специализированного транспорта и контролем на всём маршруте.</Text><div className={styles.heroActions}><CalculationRequestModal label="Рассчитать перевозку" size="lg" /></div></div></section>
